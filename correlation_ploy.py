...
plt.figure()
for f in args.files:
    corr_file = lambda s: f + '/OP' + [file for file in os.listdir(f + '/OP') if re.match(s, f)][0]
    for x_col, y_col, s in zip(args.x_column, args.y_column, args.style):
        ...
        if args.upper:
            x, y = np.loadtxt(corr_file('upper_psi_1' + str(m * n) + '_corr*'), usecols=(x_col - 1, y_col - 1), unpack=True)
